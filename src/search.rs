...
lazy_static! {
    pub static ref TORRENT_SEARCH_ENGINE: RwLock<SearchEngine> =
...
impl SearchEngine {
    ...
    pub fn search(&self, patterns: Vec<String>) -> Vec<i64> {
        ...
        let result_ids = ret.iter().map(|(id, _)| *id).collect();
...
mod tests {
    ...
    fn search_exact_keyword_works() {
        ...
        engine.insert(
            1,
            vec![
                "[喵萌奶茶屋][回转企鹅罐 Mawaru Penguindrum][BDRIP][720P][X264-10bit_AAC]"
                    .to_string(),
            ],
        );
        engine.insert(
            2,
            vec![
                "[MagicStar][回转企鹅罐 Mawaru Penguindrum][BDRIP][720P][X264-10bit_AAC]"
                    .to_string(),
            ],
        );
        engine.insert(
            3,
            vec![
                "[桜都字幕组][回转企鹅罐 Mawaru Penguindrum][BDRIP][720P][X264-10bit_AAC]"
                    .to_string(),
            ],
        );
